...
cartItems.forEach(product => {
	const template = `
            <div class="row data" data-id="${product.itemId}">
                ...
                <div class="subdiv">
                    <div class="basketcmd">
                        <a href="javascript:void(0)" class="abutton" onclick="basket.delItem(event);">삭제</a>
                    </div>
                </div>
            </div>`;
...
const basket = {
	...
	// 선택상품삭제.
	delCheckedItem() {
			document.querySelectorAll('input[type="checkbox"][name="buy"]:checked').forEach(chk => {
				const row = chk.closest('div.row');
				if (row) row.remove();
			});
			this.allItem();
		},
	// 장바구니 비우기.
	delAllItem() {
			document.querySelectorAll('div.row.data').forEach(row => row.remove());
			this.allItem();
		},
	// 전체수량과 금액을 계산해서 보여주기.
	allItem() {
